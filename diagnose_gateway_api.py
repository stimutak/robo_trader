...
def check_tcp_connection(host: str = "127.0.0.1", port: int = 4002, timeout: float = 5.0) -> tuple[bool, str]:
    ...
    try:
        ...
        if result == 0:
            print_result(
                "TCP connection",
                True,
                f"Connected in {connect_time*1000:.1f}ms"
            )
    ...
    try:
        ...
        if not ib.isConnected():
            print_result(
                "API handshake",
                False,
                "Connected but isConnected() returned False"
            )
        ...
        print_result(
            "API handshake",
            True,
            f"Connected in {connect_time:.2f}s"
        )
        ...
        print_result(
            "API handshake",
            False,
            f"TIMEOUT after {connect_time:.2f}s"
        )
    ...
    try:
        ...
        if zombie_count == 0:
            ...
            print_result(
                "Zombie connections",
                False,
                f"Found {zombie_count} CLOSE_WAIT connection(s)"
            )
            print("\n   Zombie connections:")
            for line in result.stdout.split("\n"):
                if line.strip() and not line.startswith("COMMAND"):
...
def check_gateway_api_logs() -> tuple[bool, str]:
        ...
        if log_path.exists():
            ...
            if log_files:
                print_result(
                    "API logs found",
                    True,
                    f"Found {len(log_files)} log file(s) in {log_path}"
                )
    ...
    print_result(
        "API logs found",
        False,
        "No API log files found (API logging may not be enabled)"
    )
...
async def main():
    ...
    all_passed = all(result[0] for result in [
        results['process'],
        results['port'],
        results['tcp'],
        results['api']
    ...
    if all_passed:
        ...
        if results['zombies'][0] > 0:
            ...
            print("   → Run: python3 -c 'from robo_trader.utils.robust_connection import kill_tws_zombie_connections; kill_tws_zombie_connections(4002)'")
            print()
