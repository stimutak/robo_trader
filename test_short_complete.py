...
def test_short_position_tracking():
    ...
    for symbol, pos in positions.items():
        ...
        if pos.quantity > 0:
            ...
            short_value += value
            print(f"      {symbol}: SHORT {abs(pos.quantity)} @ ${pos.avg_price:.2f} = ${value:,.0f}")
...
def test_risk_limits():
    ...
    for symbol, quantity, price in test_orders:
        ...
        if current_short_exposure + order_value <= max_short:
            ...
            print(f"      Current: ${current_short_exposure:,.0f} + ${order_value:,.0f} = ${current_short_exposure + order_value:,.0f}")
            print(f"      Limit: ${max_short:,.0f}")
            current_short_exposure += order_value
        else:
            print(f"   ❌ {symbol}: ${order_value:,.0f} - BLOCKED")
            print(f"      Would exceed limit: ${current_short_exposure + order_value:,.0f} > ${max_short:,.0f}")
