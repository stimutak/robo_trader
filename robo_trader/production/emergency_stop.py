...
class EmergencyStopManager:
    ...
    def emergency_stop(
        ...
        with self._lock:
            ...
            event = EmergencyStopEvent(
                ...
                resume_after=(
                    datetime.now() + timedelta(minutes=auto_resume_minutes)
    ...
    def check_trading_allowed(
        ...
        if self.is_stopped:
            return (
                False,
                f"Trading stopped: {self.stop_event.message if self.stop_event else 'Unknown'}",
            )
        ...
        for restriction in self.restrictions.values():
            ...
            if restriction.scope == StopScope.ALL_TRADING:
                ...
                if restriction.target == symbol:
                    return (
                        False,
                        f"Symbol {symbol} restricted: {restriction.reason.value}",
                    )
                ...
                if restriction.target == strategy:
                    return (
                        False,
                        f"Strategy {strategy} restricted: {restriction.reason.value}",
                    )
                ...
                if restriction.target == account:
                    return (
                        False,
                        f"Account {account} restricted: {restriction.reason.value}",
                    )
    ...
    def get_audit_trail(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of stop events
        """
        return [event.to_dict() for event in self.stop_history[-limit:]]
