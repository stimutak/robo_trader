...
class ExecutionConfig(BaseModel):
    ...
    position_timeout_hours: int = Field(default=24, ge=1, le=168, description="Position timeout in hours")
    order_timeout_seconds: int = Field(default=30, ge=5, le=300, description="Order timeout in seconds")
...
class RiskConfig(BaseModel):
    ...
    max_position_pct: float = Field(default=0.02, gt=0, le=0.1, description="Max position size as % of portfolio")
    max_daily_loss_pct: float = Field(default=0.005, gt=0, le=0.05, description="Max daily loss as % of portfolio")
    max_portfolio_beta: float = Field(default=1.2, gt=0, le=2.0, description="Maximum portfolio beta")
    correlation_limit: float = Field(default=0.7, ge=0, le=1.0, description="Maximum correlation between positions")
    min_volume: int = Field(default=1_000_000, ge=100_000, description="Minimum daily volume for stocks")
    min_market_cap: float = Field(default=1_000_000_000, ge=100_000_000, description="Minimum market cap in USD")
    max_sector_exposure_pct: float = Field(default=0.3, gt=0, le=0.5, description="Max sector exposure")
    max_leverage: float = Field(default=2.0, ge=1.0, le=4.0, description="Maximum account leverage")
    stop_loss_pct: float = Field(default=0.02, gt=0, le=0.1, description="Default stop loss percentage")
    take_profit_pct: float = Field(default=0.05, gt=0, le=0.2, description="Default take profit percentage")
    
    # New risk parameters for enhanced control
    max_order_notional: Optional[float] = Field(default=10_000, ge=100, description="Max notional per order")
    max_daily_notional: Optional[float] = Field(default=100_000, ge=1000, description="Max daily trading notional")
    max_open_positions: int = Field(default=20, ge=1, le=100, description="Maximum open positions")
    position_sizing_method: str = Field(default="fixed", description="Position sizing method: fixed, atr, kelly")
    use_trailing_stop: bool = Field(default=False, description="Enable trailing stop losses")
    trailing_stop_pct: float = Field(default=0.015, gt=0, le=0.1, description="Trailing stop percentage")
...
class DataConfig(BaseModel):
    ...
    feature_window: int = Field(default=100, ge=20, le=500, description="Bars for indicator calculation")
    tick_buffer: int = Field(default=10_000, ge=1000, description="Tick data buffer size")  # Renamed for consistency
    cache_ttl: int = Field(default=300, ge=60, description="Cache TTL in seconds")  # Renamed for consistency
    enable_realtime: bool = Field(default=True, description="Enable real-time data streaming")  # Renamed for consistency
    historical_days: int = Field(default=30, ge=1, le=365, description="Days of historical data to fetch")
...
class StrategyConfig(BaseModel):
    ...
    breakout_volume_factor: float = Field(default=1.5, gt=1, description="Volume factor for breakouts")
...
class Config(BaseModel):
    ...
    default_cash: float = Field(default=100_000, gt=0, description="Starting cash for paper trading")
...
def load_config_from_env() -> Config:
    ...
    config_dict = {
        ...
        "risk": {
            ...
            "max_order_notional": float(os.getenv("RISK_MAX_ORDER_NOTIONAL", "10000")) if os.getenv("RISK_MAX_ORDER_NOTIONAL") else None,
            "max_daily_notional": float(os.getenv("RISK_MAX_DAILY_NOTIONAL", "100000")) if os.getenv("RISK_MAX_DAILY_NOTIONAL") else None,
...
__all__ = ["Config", "load_config", "get_config_for_environment", "TradingMode", "Environment"]