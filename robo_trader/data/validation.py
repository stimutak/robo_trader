...
class DataValidator:
    ...
    def check_corporate_action(self, symbol: str, 
                              prev_close: float, 
                              current_price: float) -> Optional[ValidationResult]:
...
class DataHealthMonitor:
    ...
    def update(self, symbol: str) -> str:
        ...
        if symbol in self.health_status and self.health_status[symbol] != status:
            self.logger.warning(f"Data health changed for {symbol}: {self.health_status[symbol]} -> {status}")
