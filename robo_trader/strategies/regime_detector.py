...
class RegimeDetector:
    ...
    async def initialize(self) -> None:
        ...
        if self.use_ml_detection:
            ...
            self.gmm_model = GaussianMixture(
                n_components=4, covariance_type="full", random_state=42  # 4 main regimes
            )
    ...
    async def detect_ml_regime(self, symbol: str, data: pd.DataFrame) -> Tuple[MarketRegime, float]:
    ...
    def calculate_transition_probability(
        self, symbol: str, current_regime: MarketRegime, data: pd.DataFrame
    ) -> float:
        ...
        if regime_durations:
            ...
            current_regime_duration = sum(
                1 for r in reversed(history) if r.trend_regime == current_regime
            )
    ...
    async def detect_regime(
        self, symbol: str, data: pd.DataFrame, use_ensemble: bool = True
    ) -> RegimeState:
        ...
        indicators = {
            ...
            "volume_profile": (
                data["volume"].iloc[-1] / data["volume"].mean() if "volume" in data.columns else 1.0
            ),
        }

        # Calculate transition probability
        transition_prob = self.calculate_transition_probability(symbol, final_trend_regime, data)
