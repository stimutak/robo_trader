...
class BaseStrategy:
    ...
    def calculate_position_size(
        self, symbol: str, current_price: float, capital: float, market_data: Dict
    ) -> int:
...
class OrderFlowImbalanceStrategy(BaseStrategy):
    ...
    def should_buy(self, symbol: str, current_price: float, market_data: Dict) -> Tuple[bool, Dict]:
        ...
        should_buy = (
            ofi > self.config.ofi_entry_threshold
            and book_pressure > 0.2
            and self.positions.get(symbol, 0) <= 0  # Not already long
        )
...
class SpreadTradingStrategy(BaseStrategy):
    ...
    def calculate_quotes(
        ...
        inventory_ratio = (
            current_inventory / self.config.max_inventory if self.config.max_inventory > 0 else 0
        )
...
class TickMomentumStrategy(BaseStrategy):
    ...
    def should_buy(self, symbol: str, current_price: float, market_data: Dict) -> Tuple[bool, Dict]:
        ...
        should_buy = (
            tick_direction > self.config.tick_threshold
            and momentum > 1.0
            and symbol not in self.positions  # Positive momentum in bps  # Not already in position
        )
        ...
        if should_buy:
            ...
            logger.info(
                f"Tick momentum buy for {symbol}: Direction={tick_direction:.3f}, Momentum={momentum:.1f}bps"
            )
    ...
    def should_sell(
        ...
        holding_time = (
            datetime.now() - self.entry_times.get(symbol, datetime.now())
        ).total_seconds()

        # Exit conditions
        should_sell = (
            tick_direction < -self.config.tick_threshold * 0.5
            or recent_return < -self.config.stop_loss_bps  # Momentum reversal
            or recent_return > self.config.take_profit_bps  # Stop loss
            or holding_time > 60  # Take profit  # Max holding time of 60 seconds for HFT
        )
        ...
        if should_sell:
            logger.info(
                f"Tick momentum sell for {symbol}: Reason={exit_reason}, Return={recent_return:.1f}bps"
            )
...
class MicrostructureEnsembleStrategy(BaseStrategy):
    ...
    def __init__(self, config: Optional[MicrostructureConfig] = None):
        ...
        # Ensemble weights
        self.weights = {"ofi": 0.4, "spread": 0.3, "tick": 0.3}
    ...
    def should_buy(self, symbol: str, current_price: float, market_data: Dict) -> Tuple[bool, Dict]:
        ...
        spread_signal, spread_meta = self.spread_strategy.should_buy(
        ...
        metadata = {
            ...
            "sub_strategies": {"ofi": ofi_meta, "spread": spread_meta, "tick": tick_meta},
        }
    ...
    def should_sell(
        ...
        spread_signal, spread_meta = self.spread_strategy.should_sell(
            symbol, current_price, market_data
        )
    ...
    def calculate_position_size(
        ...
        ofi_size = self.ofi_strategy.calculate_position_size(
            symbol, current_price, capital, market_data
        )
        spread_size = self.spread_strategy.calculate_position_size(
            symbol, current_price, capital, market_data
        )
        tick_size = self.tick_strategy.calculate_position_size(
            symbol, current_price, capital, market_data
        )
