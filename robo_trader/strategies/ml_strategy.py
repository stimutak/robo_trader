...
from .framework import Strategy as BaseStrategy, Signal, SignalType, StrategyState
...
class MLStrategy(BaseStrategy):
    ...
    async def _generate_signals(
        ...
        for symbol in self.symbols:
            ...
            symbol_market_data = {
                ...
                "price": market_data[symbol]["close"].iloc[-1] if len(market_data[symbol]) > 0 else 0,
                "portfolio_value": 100000,  # Default, should be passed in
                "atr": market_data[symbol]["close"].rolling(14).std().iloc[-1] if len(market_data[symbol]) > 14 else 2
            }
    ...
    def calculate_technical_signals(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "momentum": "strong" if momentum_score > 0.7 else "weak" if momentum_score < 0.3 else "moderate",
    ...
    async def multi_timeframe_analysis(
        ...
        for timeframe in self.timeframes:
            if timeframe in data_dict:
                analysis = await self.analyze_timeframe(
                    symbol,
                    data_dict[timeframe],
                    timeframe
                )
