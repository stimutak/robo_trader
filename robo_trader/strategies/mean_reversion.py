        ...
        if not has_position:
            ...
            if reversion_score < -self.zscore_threshold / 2:
                return self._create_reversion_entry(
                    ...
                    feature_set=feature_set,
                    reversion_score=reversion_score,
                    target_mean=feature_set.bb_middle or data['close'].rolling(self.bb_period).mean().iloc[-1]
                )
        ...
        # Exit signals for existing positions
            ...
            if feature_set.bb_middle:
                ...
                if position.get('side') == 'long':
                    if (current_deviation >= 0 or  # Reached mean
                        reversion_score > 0.5 or  # Becoming overbought
                        holding_period >= self.max_holding_periods):  # Max holding time
                        ...
                        return Signal(
                            ...
                            metadata={
                                ...
                                'profit_pct': (current_price / position.get('entry_price', current_price) - 1) * 100
                            }
                        )
                ...
                # Short position exit conditions
                    ...
                    if (current_deviation <= 0 or  # Reached mean
                        reversion_score < -0.5 or  # Becoming oversold
                        holding_period >= self.max_holding_periods):
    ...
    def _create_reversion_entry(
        ...
        return Signal(
            ...
            metadata={
                ...
                'bb_width': (feature_set.bb_upper - feature_set.bb_lower) if feature_set.bb_upper else None,
                'rsi': feature_set.rsi
            }
        )
