...
class BacktestEngine:
    ...
    async def _execute_signal(
        ...
        if signal.signal_type == SignalType.BUY:
            ...
            if symbol in self.positions:
                ...
                margin_required = abs(signal.quantity) * execution_price * self.config.margin_requirement
            ...
            if symbol in self.positions:
                ...
                if position.quantity > 0:  # Close long
                    ...
                    proceeds = -cost + (abs(position.quantity) * position.entry_price * self.config.margin_requirement)
    ...
    def _check_exit_conditions(
        ...
        for symbol, reason in positions_to_close:
            ...
            if position.quantity > 0:
                ...
                proceeds = -abs(position.quantity) * execution_price - commission + \
                          (abs(position.quantity) * position.entry_price * self.config.margin_requirement)
    ...
    def _close_all_positions(
        ...
        for symbol in list(self.positions.keys()):
            ...
            if position.quantity > 0:
                ...
                proceeds = -abs(position.quantity) * final_price - commission + \
                          (abs(position.quantity) * position.entry_price * self.config.margin_requirement)
            ...
            logger.debug(
                "backtest.final_position_closed",
                symbol=symbol,
                pnl=pnl
            )
    ...
    def _create_result(
        ...
        statistics = {
            ...
            'avg_bars_held': np.mean([p.bars_held for p in self.closed_positions]) if self.closed_positions else 0,
            'total_commission': sum([p.commission_paid + p.exit_commission for p in self.closed_positions]),
