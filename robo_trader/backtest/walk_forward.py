...
from ..features import MomentumIndicators, TrendIndicators, VolatilityIndicators, VolumeIndicators
...
class ExecutionSimulator:
    ...
    def can_execute(
        self, quantity: int, avg_volume: float, current_volume: float
    ) -> Tuple[bool, str]:
        """
...
class WalkForwardBacktest:
    ...
    def generate_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        features["overnight_gap"] = (features["open"] - features["close"].shift(1)) / features[
            "close"
        ].shift(1)
    ...
    def create_windows(
        ...
        while current_date + timedelta(days=self.config.test_window_days) <= end_date:
            ...
            test_end = min(current_date + timedelta(days=self.config.test_window_days), end_date)
    ...
    async def _run_backtest_period(
        ...
        if self.correlation_tracker and "symbol" in data.columns:
            ...
            self.correlation_tracker.add_price_series(symbol=symbol, prices=data["close"])
            ...
            if signal != 0:
                ...
                if can_exec:
                    ...
                    if signal > 0:
                        ...
                        cash -= (
                            exec_result["fill_price"] * position_size + exec_result["commission"]
                        )
                    else:
                        if symbol in positions:
                            cash += (
                                exec_result["fill_price"] * positions[symbol]["quantity"]
        ...
        for metric in ["sharpe_ratio", "win_rate", "avg_trade"]:
            if metric in train_metrics and metric in test_metrics:
                ...
                if abs(train_val) > 1e-6:
                    stability_scores[f"{metric}_stability"] = 1 - abs(train_val - test_val) / abs(
                        train_val
                    )
                else:
    ...
    async def run_monte_carlo_validation(
        self, results: pd.DataFrame, n_simulations: int = None
    ) -> Dict[str, Any]:
        """
    ...
    def calculate_parameter_sensitivity(
        self, results: List[Dict], parameter_name: str
    ) -> Dict[str, float]:
        """
    ...
    async def run_multi_symbol_backtest(
        self, data: Dict[str, pd.DataFrame], engine: BacktestEngine, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if self.correlation_tracker:
            for symbol, df in data.items():
                self.correlation_tracker.add_price_series(symbol=symbol, prices=df["close"])
        ...
        windows = self.create_windows(
            start_date=primary_data.index[0], end_date=primary_data.index[-1]
        )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        oos_sharpes = [r["test_metrics"]["sharpe_ratio"] for r in self.window_results if r]
        oos_returns = [r["test_metrics"]["total_return"] for r in self.window_results if r]
        ...
        return {
            ...
            "stability": {
                ...
                "parameter_variance": {
                    param: np.std(values) for param, values in self.parameter_stability.items()
                },
            },
            "execution_costs": {
                "avg_slippage": np.mean(
                    [r["test_metrics"]["total_slippage"] for r in self.window_results if r]
                ),
                "avg_commission": np.mean(
                    [r["test_metrics"]["total_commission"] for r in self.window_results if r]
                ),
            },
