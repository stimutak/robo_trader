...
class StrategyManager:
        ...
        for strategy in self.strategies:
            try:
                ...
                if not result.empty and 'signal' in result.columns:
                    ...
                    confidence = result.get('confidence', pd.Series([1.0])).iloc[-1] if 'confidence' in result.columns else 1.0
        ...
        if symbol in self.position_correlations:
            for other_symbol, correlation in self.position_correlations[symbol].items():
                if abs(correlation) > self.correlation_threshold:
                    logger.warning(f"High correlation ({correlation:.2f}) between {symbol} and {other_symbol}")
                    return True
    ...
    def record_trade_result(self, strategy_name: str, symbol: str, 
                           entry_price: float, exit_price: float, success: bool) -> None:
        ...
        if strategy_name in self.strategy_performance:
            ...
            logger.debug(f"Recorded trade result for {strategy_name}: "
...
def create_default_manager() -> StrategyManager:
    ...
    manager.register_strategy(StrategyConfig(
        name="SMA_FAST",
