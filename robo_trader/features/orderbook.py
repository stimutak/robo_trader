...
class OrderBookFeatures:
    ...
    def calculate_volume_weighted_average_price(self, side: str = "both", levels: int = 5) -> float:
        ...
        if side == "bid" or side == "both":
            ...
            bid_vwap = (
                np.sum(bid_prices * bid_sizes) / np.sum(bid_sizes) if np.sum(bid_sizes) > 0 else 0
            )
        else:
            bid_vwap = 0
        ...
        if side == "ask" or side == "both":
            ...
            ask_vwap = (
                np.sum(ask_prices * ask_sizes) / np.sum(ask_sizes) if np.sum(ask_sizes) > 0 else 0
            )
        else:
            ask_vwap = 0
    ...
    def calculate_spread_metrics(self) -> Dict[str, float]:
        ...
        return {"spread": spread, "spread_pct": spread_pct, "effective_spread": effective_spread}
    ...
    def detect_order_book_anomalies(self) -> Dict[str, bool]:
        ...
        return {"wide_spread": wide_spread, "thin_book": thin_book, "one_sided": one_sided}
