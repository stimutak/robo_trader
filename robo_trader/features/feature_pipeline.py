...
class FeaturePipeline:
    ...
    def __init__(self, config: Config):
        ...
        self.feature_store = FeatureStore(db_path="feature_store.db", max_versions=10)
        ...
        self.metrics = {"features_calculated": 0, "pipeline_latency_ms": [], "feature_count": 0}
    ...
    async def calculate_features_timeseries(
        ...
        if "Close" in price_data.columns:
            ...
            price_data["bb_bandwidth"] = (
                price_data["bb_upper"] - price_data["bb_lower"]
            ...
            if "Volume" in price_data.columns:
                ...
                price_data["obv"] = (
                    np.sign(price_data["Close"].diff()) * price_data["Volume"]
                ).cumsum()
            ...
            # Volatility features
            price_data["historical_volatility"] = price_data["returns"].rolling(20).std() * np.sqrt(
                252
            )
            price_data["volatility_ratio"] = (
                price_data["historical_volatility"]
                / price_data["historical_volatility"].rolling(60).mean()
            )
    ...
    async def calculate_features(
        ...
        try:
            ...
            if self.enabled_features:
                filtered_features = {
                    k: v for k, v in all_features.items() if k in self.enabled_features
                }
            else:
            ...
            if store_features and len(features_df) > 0:
                ...
                await self.feature_store.store_features(
                    symbol=symbol, features=features_df, metadata=metadata
                )
    ...
    async def get_features_for_training(
        ...
        for symbol in symbols:
            features = await self.feature_store.get_feature_history(
                symbol=symbol, start_date=start_date, end_date=end_date, feature_names=feature_names
            )
    ...
    async def get_top_features(self, n: int = 50, model_name: Optional[str] = None) -> List[str]:
    ...
    async def update_feature_importance(
        self, importance_scores: Dict[str, float], model_name: str
    ) -> None:
        """Update feature importance from ML model."""
        await self.feature_store.update_feature_importance(
            importance_scores=importance_scores, model_name=model_name
        )
        ...
        if len(importance_scores) > 0:
            ...
            self.logger.info(f"Selected {len(selected_features)} features covering 80% importance")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        avg_latency = (
            np.mean(self.metrics["pipeline_latency_ms"])
            if self.metrics["pipeline_latency_ms"]
            else 0
        )
