...
class FeatureStore:
    ...
    async def _init_database(self) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Create indices
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_symbol_timestamp ON features (symbol, timestamp)"
            )
    ...
    async def store_features(
        self, symbol: str, features: pd.DataFrame, metadata: Optional[FeatureMetadata] = None
    ) -> str:
        ...
        try:
            ...
            if len(self.cache) > self.cache_size:
                oldest_key = min(
                    self.cache_metadata.keys(), key=lambda k: self.cache_metadata[k].timestamp
                )
    ...
    async def get_features(
        self, symbol: str, timestamp: Optional[datetime] = None, version: Optional[int] = None
                ...
                async with db.execute(query, params) as cursor:
                    ...
                    if row:
                        ...
                        if len(self.metrics["avg_retrieval_time_ms"]) > 100:
                            self.metrics["avg_retrieval_time_ms"] = self.metrics[
                                "avg_retrieval_time_ms"
                            ][-100:]
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                async with db.execute(
                    query, (symbol, start_date.isoformat(), end_date.isoformat())
                ) as cursor:
    ...
    async def get_top_features(
        self, n: int = 50, model_name: Optional[str] = None
    ...
    async def _get_next_version(self, symbol: str) -> int:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute(
                    "SELECT MAX(version) FROM features WHERE symbol = ?", (symbol,)
                ) as cursor:
                    row = await cursor.fetchone()
                    return (row[0] or 0) + 1
    ...
    async def _cleanup_old_versions(self, symbol: str) -> None:
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                # Get version count
                async with db.execute(
                    "SELECT COUNT(DISTINCT version) FROM features WHERE symbol = ?", (symbol,)
                ) as cursor:
                    row = await cursor.fetchone()
                    version_count = row[0]
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_storage_time_ms": (
                np.mean(self.metrics["avg_storage_time_ms"])
                if self.metrics["avg_storage_time_ms"]
                else 0
            ),
            "avg_retrieval_time_ms": (
                np.mean(self.metrics["avg_retrieval_time_ms"])
                if self.metrics["avg_retrieval_time_ms"]
                else 0
            ),
