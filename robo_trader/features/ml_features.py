        ...
        try:
            ...
            if len(returns.columns) >= 2:
                ...
                if 'SPY' in returns.columns and symbol in returns.columns:
                    features['correlation_spy'] = returns[symbol].tail(20).corr(
                        returns['SPY'].tail(20)
                    )
            ...
            if trades and quotes:
                ...
                for trade in trades[-10:]:  # Last 10 trades
                    ...
                    if trade_time:
                        nearest_quote = min(
                            quotes,
                            key=lambda q: abs(q.get('timestamp', 0) - trade_time)
                        )
            ...
            if len(trades) >= 10:
                ...
                if len(prices) == len(volumes):
                    ...
                    if len(price_changes) > 0 and cumulative_volume.sum() > 0:
                        ...
                        features['price_impact'] = np.sum(np.abs(price_changes)) / \
    ...
    def calculate_statistical_arbitrage_signals(
        ...
        try:
            ...
            try:
                ...
                features[f'pair_{symbol1}_{symbol2}_cointegrated'] = 1.0 if adf_result[1] < 0.05 else 0.0
