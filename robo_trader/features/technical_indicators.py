...
class MomentumIndicators(BaseFeatureCalculator):
    ...
    def _calculate_cci(self, high: pd.Series, low: pd.Series,
                       close: pd.Series, period: int = 20) -> float:
...
class VolatilityIndicators(BaseFeatureCalculator):
    ...
    def _calculate_bollinger_bands(self, prices: pd.Series, 
                                   period: int = 20, num_std: float = 2) -> tuple:
...
class VolumeIndicators(BaseFeatureCalculator):
    ...
    def calculate(self, data: pd.DataFrame) -> Dict[str, FeatureValue]:
        ...
        mfi = self._calculate_mfi(data['high'], data['low'], data['close'], 
                                 data['volume'], self.window_size)
    ...
    def _calculate_mfi(self, high: pd.Series, low: pd.Series,
                      close: pd.Series, volume: pd.Series, period: int = 14) -> float:
