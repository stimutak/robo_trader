...
class FeatureEngine(DataSubscriber):
    ...
    async def calculate_features(self, symbol: str) -> Optional[FeatureSet]:
        ...
        try:
            ...
            if symbol in self.tick_data and self.tick_data[symbol]:
                ...
                if bid_sizes and ask_sizes:
                    ...
                    if total_bid + total_ask > 0:
                        features.bid_ask_imbalance = (total_bid - total_ask) / (total_bid + total_ask)
            ...
            # Momentum indicators
            if len(df) >= 20:
                features.momentum_1d = (df['close'].iloc[-1] / df['close'].iloc[-12] - 1) * 100  # Approx 1 day
                features.roc = ((df['close'].iloc[-1] - df['close'].iloc[-20]) / df['close'].iloc[-20]) * 100
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        avg_calc_time = np.mean(self.metrics['calculation_time_ms']) if self.metrics['calculation_time_ms'] else 0
        ...
        return {
            ...
            'cache_hit_rate': self.metrics['cache_hits'] / (self.metrics['cache_hits'] + self.metrics['cache_misses'] + 0.0001),
            'symbols_tracked': len(self.price_data),
            'features_per_symbol': len(FeatureSet.__dataclass_fields__) - 2  # Exclude timestamp and symbol
        }