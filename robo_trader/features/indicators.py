...
class TechnicalIndicators:
    ...
    def mfi(self, df: pd.DataFrame, period: int = 14) -> Optional[float]:
        ...
        try:
            ...
            money_flow = pd.DataFrame(
                {"typical_price": typical_price, "raw_money_flow": raw_money_flow}
            )

            money_flow["price_change"] = money_flow["typical_price"].diff()
            money_flow["positive_flow"] = money_flow["raw_money_flow"].where(
                money_flow["price_change"] > 0, 0
            )
            money_flow["negative_flow"] = money_flow["raw_money_flow"].where(
                money_flow["price_change"] < 0, 0
            )
    ...
    def keltner_channels(
        ...
        try:
            ...
            return {"upper": upper, "middle": middle.iloc[-1], "lower": lower}
