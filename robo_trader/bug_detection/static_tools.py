    ...
    def _check_availability(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [self.tool_name, "--version"], capture_output=True, text=True, timeout=10
            )
...
class BanditAnalyzer(StaticAnalysisTool):
    ...
    async def _analyze_file(self, file_path: Path) -> List[BugReport]:
        ...
        try:
            cmd = ["bandit", "-f", "json", "-q", str(file_path)]  # Quiet mode
...
class StaticAnalysisManager:
    ...
    async def analyze_file(
        self, file_path: Path, tools: Optional[List[str]] = None
    ) -> List[BugReport]:
    ...
    async def analyze_directory(
        self, directory: Path, tools: Optional[List[str]] = None
    ) -> List[BugReport]:
