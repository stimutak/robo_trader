...
class BugDetectionConfig:
    ...
    # Runtime monitoring
    log_file_paths: List[str] = field(default_factory=lambda: [
        "logs/",
        "*.log"
    ])
...
class StaticAnalyzer:
                ...
                if isinstance(node.value, (int, float)):
                    ...
                    if node.value in [0, 1, 100, 1000, 10000]:
                        ...
                        if parent and hasattr(parent, 'id'):
                            if any(keyword in str(parent.id).lower() for keyword in 
                                   ['price', 'quantity', 'amount', 'size', 'risk']):
                                bugs.append(BugReport(
    ...
    def _check_security_issues(self, content: str, file_path: Path) -> List[BugReport]:
        ...
        dangerous_patterns = [
            (r'eval\s*\(', "Use of eval() function", BugSeverity.CRITICAL),
            (r'exec\s*\(', "Use of exec() function", BugSeverity.CRITICAL),
            (r'os\.system\s*\(', "Use of os.system()", BugSeverity.HIGH),
            (r'subprocess\.call\s*\(', "Use of subprocess.call()", BugSeverity.HIGH),
            (r'pickle\.loads?\s*\(', "Use of pickle.loads()", BugSeverity.HIGH),
                ...
                for pattern, severity in self.error_patterns.items():
                    if re.search(pattern, line, re.IGNORECASE):
                        bugs.append(BugReport(
                            ...
                            metadata={"pattern": pattern, "timestamp": datetime.now().isoformat()}
                        ))
...
async def main():
    ...
    parser.add_argument("--severity", choices=[s.value for s in BugSeverity], 
    ...
    if args.watch:
        ...
        if args.severity:
            ...
            severity_order = [BugSeverity.CRITICAL, BugSeverity.HIGH, 
                            BugSeverity.MEDIUM, BugSeverity.LOW, BugSeverity.INFO]
            min_index = severity_order.index(min_severity)
            bugs = [bug for bug in bugs 
                   if severity_order.index(bug.severity) <= min_index]
