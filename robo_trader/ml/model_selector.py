...
class ModelSelector:
    ...
    async def select_best_model(
        ...
        for name, model_info in self.available_models.items():
            if require_recent:
                trained_at = datetime.fromisoformat(
                    model_info.get("trained_at", "2000-01-01")
                )
            ...
            if score < self.min_performance_score:
                logger.debug(
                    f"Skipping low-performance model: {name}",
                    score=score
                )
                continue
        ...
        if validation_data is not None:
            ...
            for name, model_info in candidates:
                val_metrics = await self.model_trainer.validate_model(
                    model_info,
                    features,
                    target
                )
        ...
        if validation_data is not None:
            ...
            best_name, best_model = max(
                candidates,
                key=lambda x: x[1].get("metrics", {}).get(selection_criteria, 0)
            )
    ...
    async def compare_models(
        ...
        for model_type in model_types:
            ...
            if prediction_type == PredictionType.REGRESSION:
                ...
                result["direction_accuracy"] = model_info["metrics"].get(
        ...
        # Add ranking
        comparison_df["rank"] = comparison_df["test_score"].rank(
            ascending=False,
            method="min"
        ).astype(int)
    ...
    def get_model_confidence(self, model_info: Dict) -> float:
        ...
        if model_info.get("prediction_type") == "regression":
            dir_acc = model_info.get("metrics", {}).get(
