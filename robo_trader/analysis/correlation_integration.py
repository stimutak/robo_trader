...
class CorrelationBasedPositionSizer:
    ...
    async def calculate_position_size(
        ...
        total_correlated_exposure = sum(
            pc['exposure'] * abs(pc['correlation'])
            for pc in position_correlations
        )
        ...
        if max_corr > self.max_correlation:
            reduction_factor = 1 - (max_corr - self.max_correlation) * self.correlation_penalty_factor
            reduction_factor = max(0.2, reduction_factor)  # Minimum 20% of original size
            adjusted_size = int(base_size * reduction_factor)
            reasons.append(f"Reduced by {(1-reduction_factor)*100:.1f}% due to {max_corr:.2f} correlation")
    ...
    def get_metrics(self) -> Dict[str, any]:
        ...
        return {
            ...
            'avg_correlation': np.mean(self.metrics['avg_correlation'])
                              if self.metrics['avg_correlation'] else 0,
    ...
    async def _update_loop(self, interval: int):
        ...
        while self.running:
            try:
                ...
                if high_corr:
                    self.logger.warning(
                        f"High correlations detected: {high_corr[:3]}"
                    )
