    ...
    async def allocate_capital(self) -> Dict[str, float]:
        ...
        for strategy_name, weight in weights.items():
            if strategy_name in self.allocations:
                ...
                self.allocations[strategy_name].allocated_capital = self.total_capital * weight
    ...
    def _apply_weight_constraints(self, weights: Dict[str, float]) -> Dict[str, float]:
        ...
        for strategy_name, weight in weights.items():
            ...
            constrained_weight = max(
                self.min_strategy_weight, min(self.max_strategy_weight, weight)
            )
        ...
        if total_weight > 0:
            constrained_weights = {
                name: weight / total_weight for name, weight in constrained_weights.items()
            }
