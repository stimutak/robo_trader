...
class PerformanceAnalyzer:
    ...
    def maximum_drawdown(
        ...
        # Calculate duration
        duration = (max_dd_end - max_dd_start).days if hasattr(max_dd_end, 'date') else len(
            returns.loc[max_dd_start:max_dd_end]
        )
    ...
    def alpha(
        ...
        return strategy_return - (self.risk_free_rate + beta_coeff * (benchmark_return - self.risk_free_rate))
    ...
    def performance_summary(
        ...
        if benchmark_returns is not None:
            summary.update({
                ...
                "information_ratio": round(self.information_ratio(returns, benchmark_returns), 3),
    ...
    def drawdown_analysis(self, returns: pd.Series, top_n: int = 5) -> pd.DataFrame:
        ...
        for date, dd in drawdown.items():
            if dd < 0 and not in_drawdown:
                ...
                # End of drawdown
                in_drawdown = False
                duration = (date - start_date).days if hasattr(date, 'date') else len(
                    returns.loc[start_date:date]
                )
        ...
        if in_drawdown:
            duration = (returns.index[-1] - start_date).days if hasattr(returns.index[-1], 'date') else len(
                returns.loc[start_date:]
            )
