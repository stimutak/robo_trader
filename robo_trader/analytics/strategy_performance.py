    ...
    def time_attribution(
        ...
        for period, ret in period_returns.items():
            period_data = returns.loc[
                ...
                (returns.index < period + pd.DateOffset(months=1))
            ]
    ...
    async def track_strategy_performance(
        ...
        if len(returns) > 60:
            ...
            performance["rolling_metrics"] = {
                "sharpe": rolling_metrics["rolling_sharpe"].dropna().tolist()[-10:] if "rolling_sharpe" in rolling_metrics.columns else [],
                "volatility": rolling_metrics["rolling_volatility"].dropna().tolist()[-10:] if "rolling_volatility" in rolling_metrics.columns else [],
                "drawdown": rolling_metrics["rolling_max_dd"].dropna().tolist()[-10:] if "rolling_max_dd" in rolling_metrics.columns else []
            }
        else:
            performance["rolling_metrics"] = {
                "sharpe": [],
...
class MLStrategyPerformance:
    ...
    async def track_model_performance(
        ...
        if predictions_aligned.dtype == bool or predictions_aligned.dtype == int:
            ...
            precision = ((predictions_aligned == 1) & (actual_returns_aligned > 0)).sum() / (predictions_aligned == 1).sum() if (predictions_aligned == 1).sum() > 0 else 0
            recall = ((predictions_aligned == 1) & (actual_returns_aligned > 0)).sum() / (actual_returns_aligned > 0).sum() if (actual_returns_aligned > 0).sum() > 0 else 0
            ...
            direction_accuracy = (np.sign(predictions_aligned) == np.sign(actual_returns_aligned)).mean()
    ...
    def compare_models(
        ...
        for strategy in ml_strategies:
            if strategy in self.tracker.strategy_results:
                ml_metrics = self.tracker.strategy_results[strategy].get("metadata", {}).get("ml_metrics", {})
