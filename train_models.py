    ...
    for symbol, df in data.items():
        ...
        if features is not None and not features.empty:
            ...
            if len(features) == 1:
                ...
                features_row = {k: v for k, v in features_row.items() 
...
async def train_models(features_df: pd.DataFrame, targets_df: pd.Series, 
                      feature_columns: list, config: Config):
    """Train ML models.
...
async def validate_models(trained_models: dict, features_df: pd.DataFrame, 
                         targets_df: pd.Series, feature_columns: list):
