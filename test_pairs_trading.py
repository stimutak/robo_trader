...
async def test_cointegration_pairs_strategy():
    ...
    for pair in pairs:
        ...
        print(f"    {pair[0]}-{pair[1]}: corr={pair_stats.correlation:.3f}, "
    ...
    print(f"  Portfolio: {summary['active_pairs']} active pairs, "
    ...
    for strategy in strategies:
        try:
            if isinstance(strategy, CointegrationPairsStrategy):
                ...
                print(f"    {strategy.name:30} | Scores: {len(scores):2} | Avg: {np.mean(list(scores.values())):.2f}")
...
async def test_signal_generation():
    ...
    for scenario in test_scenarios:
        ...
        print(f"  {scenario['description']:35} | Signal: {signal_type:15} | Confidence: {confidence:.2f}")
