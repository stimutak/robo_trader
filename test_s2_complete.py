...
async def test_full_smart_execution():
    ...
    params = ExecutionParams(algorithm=ExecutionAlgorithm.VWAP, duration_minutes=5, slice_count=3)

    plan = await smart_executor.create_execution_plan(
        symbol="AAPL", side="BUY", quantity=2000, params=params
    )
...
async def test_all_algorithms():
    ...
    for algo, (quantity, duration) in algorithms.items():
        params = ExecutionParams(algorithm=algo, duration_minutes=duration)
        ...
        is_valid = (
            plan.algorithm == algo and plan.total_quantity == quantity and len(plan.slices) > 0
        )
...
async def test_execution_stats():
    ...
    for i in range(3):
        params = ExecutionParams(
            algorithm=ExecutionAlgorithm.TWAP, duration_minutes=1, slice_count=2
        )
