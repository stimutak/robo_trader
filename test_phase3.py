...
from robo_trader.strategies.framework import Strategy, Signal, SignalType, StrategyState
...
from robo_trader.backtest.metrics import calculate_metrics, PerformanceMetrics
...
async def test_backtesting_engine():
    ...
    try:
        ...
        data_pipeline.get_historical_data = AsyncMock(
            return_value=generate_test_data("TEST", 500)
        )
        
        feature_engine = Mock()
        feature_engine.calculate_features = AsyncMock(
            return_value=create_mock_features()
        )
...
def test_performance_metrics():
    ...
    try:
        ...
        trades = pd.DataFrame({
            ...
            'return_pct': [0.01, -0.005, 0.02, -0.003, 0.015, -0.01, 0.025, 0.005, -0.0075, 0.03],
...
async def test_integration():
    ...
    try:
        ...
        historical_data = {
            "TEST": generate_test_data("TEST"),
            "AAPL": generate_test_data("AAPL")
        }
        
        await strategy.initialize(historical_data)
        
        features = {
            "TEST": create_mock_features(),
            "AAPL": create_mock_features()
        }
