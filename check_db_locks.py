...
def check_database_locks(db_path: str = "trading_data.db") -> dict:
    ...
    try:
        ...
        result = subprocess.run(["lsof", db_path], capture_output=True, text=True, timeout=10)
        ...
        if result.returncode == 0:
            ...
            if len(lines) > 1:  # Header + data
                ...
                return {"status": "locked", "processes": processes, "count": len(processes)}
            else:
...
def kill_process_safe(pid: str, force: bool = False) -> dict:
    ...
    try:
        ...
        result = subprocess.run(
            ["kill", f"-{signal_type}", pid], capture_output=True, text=True, timeout=5
        )

        if result.returncode == 0:
            return {
                "status": "success",
                "message": f"Process {pid} terminated with SIG{signal_type}",
            }
        else:
