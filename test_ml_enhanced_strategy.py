...
from robo_trader.strategies.regime_detector import RegimeDetector, MarketRegime
...
async def test_ml_enhanced_strategy():
    ...
    for regime_type, regime_name in regimes_to_test:
        ...
        mock_regime = RegimeState(
            ...
            volatility_regime=MarketRegime.NEUTRAL if regime_type not in [MarketRegime.HIGH_VOL, MarketRegime.EXTREME_VOL] else regime_type,
            confidence=0.8,
        ...
        risk_params = strategy.regime_risk_params.get(regime_type, {"stop_loss": 0.02, "take_profit": 0.05})
...
async def test_position_sizing():
    ...
    for scenario in scenarios:
        ...
        regime = RegimeState(
            ...
            volatility_regime=MarketRegime.HIGH_VOL if scenario['volatility'] > 70 else MarketRegime.NEUTRAL,
            confidence=0.8,
        ...
        print(f"   Regime Adjustment: {strategy.regime_size_adjustments.get(scenario['regime'], 1.0):.1f}x")
