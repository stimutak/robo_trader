...
async def test_performance_attribution():
    ...
    symbol_returns = pd.DataFrame({
        symbol: generate_strategy_returns(252) 
...
async def test_strategy_tracker():
    ...
    print("\n✅ Strategy Comparison:")
    print(comparison[['strategy', 'annual_return', 'sharpe_ratio', 'max_drawdown', 'overall_score']].to_string())
...
async def test_ml_performance():
    ...
    predictions = pd.Series(
        np.random.choice([0, 1], n_days, p=[0.45, 0.55]),
        index=dates
    )
    ...
    for model_name in models:
        ...
        if model_name == "RandomForest":
            model_predictions = pd.Series(
                np.random.choice([0, 1], n_days, p=[0.48, 0.52]),
                index=dates
            )
        elif model_name == "LightGBM":
            model_predictions = pd.Series(
                np.random.choice([0, 1], n_days, p=[0.43, 0.57]),
                index=dates
            )
        ...
        print(f"   Direction Accuracy: {ml_metrics.get('direction_accuracy', ml_metrics.get('accuracy', 0)):.1%}")
