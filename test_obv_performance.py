    ...
    for size in [10, 100, 1000]:
        ...
        if vectorized_result is not None and iterative_result is not None:
            ...
            if diff < 1e-6:
                ...
                print(
                    f"✗ Size {size}: Results differ! Vectorized: {vectorized_result}, Iterative: {iterative_result}"
                )
                return False
        else:
            print(
                f"✗ Size {size}: One or both results are None! Vectorized: {vectorized_result}, Iterative: {iterative_result}"
            )
            return False
...
def test_performance():
    ...
    for size in [1000, 5000, 10000]:
        ...
        print(
            f"Size {size:5d}: Vectorized: {vectorized_time*1000:.2f}ms, "
