...
def calculate_advanced_features(df):
    ...
    for fast, slow in [(5, 20), (10, 30), (20, 50)]:
        features[f"momentum_{fast}_{slow}"] = (
            df["close"].rolling(fast).mean() / df["close"].rolling(slow).mean()
        )
...
def main():
    ...
    ensemble_model, test_acc = train_ensemble_model(X_train_scaled, y_train, X_test_scaled, y_test)
    ...
    cv_scores = cross_val_score(
        ensemble_model, X_train_scaled, y_train, cv=tscv, scoring="accuracy"
    )
