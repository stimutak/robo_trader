...
def test_correlation_tracker():
    ...
    # Initialize tracker
    tracker = CorrelationTracker(lookback_days=60, min_observations=30, correlation_threshold=0.7)
...
def test_position_sizing(tracker):
    ...
    print(
        f"  Correlation-Adjusted Concentration: {concentration['correlation_adjusted_concentration']:.3f}"
    )
...
async def test_async_manager(tracker, sizer):
    ...
    manager = AsyncCorrelationManager(correlation_tracker=tracker, position_sizer=sizer)
    ...
    adjusted_size, reason = await manager.get_adjusted_position_size(
        symbol="META", base_size=100, current_positions=positions, portfolio_value=portfolio_value
    )
