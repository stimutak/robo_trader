...
from robo_trader.monitoring.performance import PerformanceMetrics, PerformanceMonitor
...
async def simulate_trading_activity():
    ...
    for cycle in range(5):
        ...
        print(
            f"Metrics exported: {metrics.total_symbols_processed} symbols, "
            f"{metrics.total_orders_placed} orders, {metrics.total_trades_executed} trades"
        )
        print(
            f"Average latencies - Data: {metrics.data_fetch_latency:.1f}ms, "
            f"Signal: {metrics.signal_generation_latency:.1f}ms, "
            f"Order: {metrics.order_execution_latency:.1f}ms"
        )
        print(
            f"Throughput - {metrics.symbols_per_second:.1f} symbols/s, "
