...
def test_market_hours():
    ...
    for dt, expected_session, expected_open, expected_extended in test_cases:
        ...
        print(
            f"  Session: {session} (expected: {expected_session}) {'✓' if session == expected_session else '✗'}"
        )
        print(
            f"  Is Open: {is_open} (expected: {expected_open}) {'✓' if is_open == expected_open else '✗'}"
        )
        print(
            f"  Extended: {is_extended} (expected: {expected_extended}) {'✓' if is_extended == expected_extended else '✗'}"
        )
...
def test_websocket_updates():
    ...
    for symbol, price in zip(symbols, prices):
        ws_manager.send_market_update(
            symbol=symbol, price=price, bid=price - 0.02, ask=price + 0.02, volume=1000000
        )
    ...
    ws_manager.send_performance_update(
        {"total_pnl": 1820.00, "daily_pnl": 320.00, "win_rate": 0.65, "sharpe_ratio": 1.45}
    )
