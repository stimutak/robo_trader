...
def sample_data():
    ...
    target_regression = pd.Series(
        features.iloc[:, :5].mean(axis=1) + 0.1 * np.random.randn(n_samples), name="target"
    )

    # Create synthetic target (classification)
    target_classification = pd.Series(
        (target_regression > target_regression.median()).astype(int), name="target_class"
    )
...
class TestModelTrainer:
    ...
    async def test_validate_model(self, config, temp_model_dir, sample_data):
        ...
        validation_metrics = await trainer.validate_model(
            model_info, features.iloc[500:], target_regression.iloc[500:]
        )
    ...
    def test_get_model_and_params(self, config, temp_model_dir):
        ...
        model, params = trainer._get_model_and_params(
            ModelType.RANDOM_FOREST, PredictionType.REGRESSION
        )
        ...
        # Test XGBoost
        model, params = trainer._get_model_and_params(
            ModelType.XGBOOST, PredictionType.CLASSIFICATION
        )
        ...
        # Test LightGBM
        model, params = trainer._get_model_and_params(ModelType.LIGHTGBM, PredictionType.REGRESSION)
    ...
    def test_feature_importance_extraction(self, config, temp_model_dir, sample_data):
        ...
        importance = trainer._get_feature_importance(
            model, features.columns.tolist(), ModelType.RANDOM_FOREST
        )
    ...
    def test_invalid_model_type(self, config, temp_model_dir):
        ...
        with pytest.raises(ValueError, match="Unknown model type"):
            trainer._get_model_and_params(
                "invalid_model", PredictionType.REGRESSION  # Invalid model type
            )
    ...
    async def test_empty_data(self, config, temp_model_dir):
        ...
        with pytest.raises((ValueError, IndexError)):
            await trainer.train_model(
                empty_features, empty_target, ModelType.RANDOM_FOREST, PredictionType.REGRESSION
            )
