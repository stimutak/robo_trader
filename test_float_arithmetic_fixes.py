...
def test_precision_errors():
    ...
    new_total = sum(float(PrecisePricing.calculate_notional(p["qty"], p["price"])) for p in positions)
...
def test_edge_cases():
    ...
    print(f"Commission: ${float(PrecisePricing.to_decimal(commission_per_share) * PrecisePricing.to_decimal(shares) * PrecisePricing.to_decimal(2)):.2f}")
