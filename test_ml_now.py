...
def test_model():
    ...
    n_samples = 10
    X_test = pd.DataFrame(np.random.randn(n_samples, len(feature_names)), columns=feature_names)
    ...
    print(f"\n📈 Statistics:")
    print(
        f"   High confidence predictions: {total_high_conf}/{n_samples} ({total_high_conf/n_samples*100:.0f}%)"
    )
