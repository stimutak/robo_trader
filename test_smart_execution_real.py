...
async def test_real_market_data():
    ...
    async with ibkr_client.pool.acquire() as ib:
        ...
        adaptive_params = ExecutionParams(
            algorithm=ExecutionAlgorithm.ADAPTIVE, duration_minutes=20, urgency=0.7
        )

        plan = await smart_executor.create_execution_plan(
            symbol="AAPL", side="SELL", quantity=5000, params=adaptive_params
        )
        ...
        paper_executor = PaperExecutor(
            slippage_bps=2.0, smart_executor=smart_executor, use_smart_execution=True
        )
