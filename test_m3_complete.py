...
def generate_synthetic_data(n_samples=2000, n_features=20):
    ...
    data['volume_ratio'] = data['volume'] / pd.Series(data['volume']).rolling(20).mean().fillna(data['volume'][0]).values
...
async def test_hyperparameter_tuning():
    ...
    improvement = tuned_result['metrics']['test_accuracy'] - untuned_result['metrics']['test_accuracy']
...
async def test_ensemble_model():
    ...
    print(f"   Direction Accuracy: {ensemble_result['ensemble_metrics']['test_direction_accuracy']:.3f}")
...
async def test_model_persistence():
    ...
    if models:
        latest = models[-1]
        print(f"   Latest: {latest.get('model_type')} - Score: {latest.get('metrics', {}).get('test_score', 0):.3f}")
